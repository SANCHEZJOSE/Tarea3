...
int contProcesosAuto=0,reinicio=300;/*Contador de procesos automaticos 
...
void procesoUsr(int *fn,int numPorts, Matrices& matriz){
	...
	Ethernet ethernet;
	if(opcion1==0)
		opcion1=readPort(in,opcion,2,TIME_OUT);//si dentro del time out no ingresa opcion el sistema vuelve a esperar
	if(opcion1>0){
		if (opcion2==0)
		{
			...
			fflush(stdin);
			opcion[opcion2-1]='\0';
			opcNum = atoi((char*)opcion)-1;//transforma el string a numero -1
			memset(opcion, 0 , sizeof(BYTE)*3);
			estadonodo=false;
		}
		if (opcion2>0)
		{	
			if ((opcNum<15 && opcNum>-1))
			{
				estadonodo=EstadoNodo(opcNum,numPorts,matriz);
			}
			if (estadonodo )
			{
				if(opcion3==0){
				//printf("%s\n", estadonodo ? "true" : "false");
				fflush(stdin);
				if (mostrarpeticion)
				{
				...
				if (opcion3>0)
				{
					...
					paquete.ttl=ttl;
					mensaje[opcion3-1]='\0';
					int puerto=mejorPuertoDestino(opcNum,numPorts,matriz);//se selecciona el mejor puerto
					for (int m = 0; m < 6; ++m)
					{
						ethernet.MACO[m]=macNodo[m];//se copia la mac del nodo en Ethernet(origen)
						ethernet.MACD[m]=matriz.mac[opcNum][m];//se copia la mac de destino en Ethernet
					}
					//strcpy( (char*)ethernet.MACO, (char*)macNodo);
					//strcpy( (char*)ethernet.MACD , (char*)matriz.mac[opcNum]);
